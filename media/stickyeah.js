...
/*
		...
		var getStopperHeight = function() {
			...
			if ($stickyStopper && $stickyStopper.length) {
				$stickyStopper.not('.stickyeah_disabled').each(function(){
					var $this = $(this);
					stopperHeight += $this.data('stickyeah-offset') + ($this.data('clone').outerHeight() || 0);
		...
		var stick = function() {
			...
			if ($stickyPusher && $stickyPusher.length) {
				getPusherTop();
				var stickyHeight = $stickyClone.outerHeight() + stickyTopNew*2;
		...
		var listenHeight = function() {
			...
			if (!$sticky.data('disabled')) {
				...
				stickyHeight = $stickyClone.outerHeight() + $sticky.data('stickyeah-offset') * 2;
